...
def run_gender_check(publication_data_zip):
	url = 'https://s3-us-west-2.amazonaws.com/iqoqo.demo/' + publication_data_zip
	wget.download(url, './' + publication_data_zip)  

	if os.path.exists(publication_data_zip):
		with ZipFile(publication_data_zip, 'r') as zip:
			publication_data = zip.extract(publication_data_zip.replace(".zip", ""))
		os.remove(publication_data_zip)
		
	if os.path.exists(publication_data):
...
def set_credentials():
    if os.getenv('IQOQO_LOGIN_USER') is None:
        os.environ['IQOQO_LOGIN_USER'] = input('user: ')
    if os.getenv('IQOQO_LOGIN_PASSWORD') is None:
        os.environ['IQOQO_LOGIN_PASSWORD'] = getpass('password: ')

def main():
	set_credentials()
	p = Pool()
	p.map(run_gender_check, ['articles1.csv.zip', 'articles2.csv.zip', 'articles3.csv.zip'])
